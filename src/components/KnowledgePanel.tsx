  ...
  return <div className="bg-card rounded-lg border border-border overflow-hidden">
      ...
      <div className="p-4">
        ...
        <div className="flex gap-3 mb-4">
          <button onClick={() => navigate("/wiki/charchit-sharma")} className="flex flex-col items-center gap-1 px-4 py-2 rounded-full transition-colors bg-secondary">
            <Globe className="w-5 h-5 text-primary" />
            <span className="text-xs text-muted-foreground">Wikipedia</span>
          </button>
          <button className="flex flex-col items-center gap-1 px-4 py-2 rounded-full transition-colors bg-secondary">
            <Share2 className="w-5 h-5 text-primary" />
            <span className="text-xs text-muted-foreground">Share</span>
          </button>
          <button className="flex flex-col items-center gap-1 px-4 py-2 rounded-full transition-colors bg-muted">
        ...
        <div className="mb-4">
          ...
          <div className="flex gap-3 bg-secondary">
            {socialLinks.map(social => <a key={social.label} href={social.href} target="_blank" rel="noopener noreferrer" aria-label={social.label} className="w-9 h-9 rounded-full flex items-center justify-center transition-colors bg-secondary">
                <social.icon className="w-4 h-4 text-muted-foreground" />
              </a>)}
            <a href="https://x.com/heyimcharchit" target="_blank" rel="noopener noreferrer" aria-label="X (Twitter)" className="w-9 h-9 rounded-full flex items-center justify-center transition-colors bg-secondary">
        ...
        <div className="flex flex-wrap gap-2">
          {tags.map(tag => <span key={tag} className="px-3 py-1 text-xs rounded-full text-muted-foreground cursor-pointer transition-colors bg-secondary">
              {tag}
            </span>)}
        </div>
      </div>
